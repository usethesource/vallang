node {
  try { 
    ...
    withMaven(maven: 'M3', jdk: 'jdk-oracle-8', options: [artifactsPublisher(disabled: true), junitPublisher(disabled: false)] ) {
        ...
        stage ('sonar cloud') {
            sh "mvn -DskipTests sonar:sonar -Dsonar.branch.name=${GIT_LOCAL_BRANCH}  -Dsonar.projectKey=usethesource_vallang -Dsonar.organization=usethesource  -Dsonar.host.url=https://sonarcloud.io -Dsonar.login=${VALLANG_SONAR_CLOUD}"
        }
