 ...
 * @deprecated this reader is only for the old IValue format. The new {@link IValueInputStream} should be used, it even switches back to this class in case an old file is encountered.
 */
@Deprecated
public class BinaryReader implements Closeable {
	...
	private INode readKeywordedNode(int header) throws IOException{
		...
		io.usethesource.capsule.Map.Transient<String, IValue> kwParams = io.usethesource.capsule.Map.Transient.of();
	...
	private INode readAnnotatedNode(int header) throws IOException{
		...
		io.usethesource.capsule.Map.Transient<String, IValue> annotations = io.usethesource.capsule.Map.Transient.of();
