...
public class ATermReader extends AbstractBinaryReader {
    ...
    public IValue read(IValueFactory factory, TypeStore store, Type type, InputStream stream) throws FactParseError, IOException {
        return new Reader(factory, store).read(type, stream);
    }

    /**
     * local class handles temporary state for a single read
     */
    private static class Reader {
        ...
        private Reader(IValueFactory vf, TypeStore ts) {
            this.vf = vf;
            this.ts = ts;
        }

        private IValue read(Type type, InputStream stream) throws FactParseError, IOException  {
            int firstToken;
            do {
