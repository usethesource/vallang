  ...
  public void testLub() {
    ...
    for (Type t1 : allTypes) {
      if (!t1.isAliased() && t1.lub(TypeFactory.getInstance().voidType()) != t1) {
        System.err.println(t1 + " lub void is not " + t1 + "? its "
            + t1.lub(TypeFactory.getInstance().voidType()));
        fail("void should be bottom: " + t1 + ".lub = "
            + t1.lub(TypeFactory.getInstance().voidType()));
      }
    ...
    for (Type t1 : allTypes) {
      for (Type t2 : allTypes) {
        if (t1.toString().equals(t2.toString())) {
          if (t1 != t2) {
            System.err.println(
                "Type descriptors should be canonical:" + t1.toString() + " == " + t2.toString());
          }
        }
        if (count++ > 10000) {
          return;
