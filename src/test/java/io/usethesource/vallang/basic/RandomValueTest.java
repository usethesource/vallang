...
import io.usethesource.vallang.random.RandomNumberGenerator;
import io.usethesource.vallang.random.RandomRationalGenerator;
import io.usethesource.vallang.random.RandomRealGenerator;
...
import io.usethesource.vallang.random.DataGenerator;
import io.usethesource.vallang.random.RandomIntegerGenerator;
 ...
 * axiom-based testing or property-based testing).
 * 
...
public final class RandomValueTest {
  ...
  public RandomValueTest(final IValueFactory vf) {
    this.vf = vf;
  }
  ...
  public void setUp() throws Exception {
    ...
    intTestSet = Arrays.asList(vf.integer(0), vf.integer(1), vf.integer(-1), vf.integer(2),
        vf.integer(-2), vf.integer(Long.MAX_VALUE), vf.integer(Long.MIN_VALUE),
    ...
    ratTestSet = Arrays.asList(vf.rational(0, 1), vf.rational(1, 1), vf.rational(-1, 1),
        vf.rational(1, 2), vf.rational(2, 1), vf.rational(-1, 2), vf.rational(-2, 1),
  ...
  protected void assertApprox(IReal l, IReal r) {
    assertTrue("Expected ~" + l + " got " + r + " (diff magnitude "
        + ((IReal) l.subtract(r).abs()).scale() + ")", approxEqual(l, r));
  }
  ...
  private class IValueOutputStreamWrapper implements IValueBinaryWriter {
    ...
    @Override
    public void write(IValue value, OutputStream stream, boolean compression) throws IOException {
      write(value, stream);
    }

    @Override
    public void write(IValue value, OutputStream stream, TypeStore typeStore) throws IOException {
      try (IValueOutputStream writer = new IValueOutputStream(stream, vf, IValueOutputStream.CompressionRate.Normal)) {
        writer.write(value);
      }
    }

    @Override
    public void write(IValue value, OutputStream stream, boolean compression, TypeStore typeStore)
        throws IOException {
  ...
  private void ioHelperText(String io, IValueTextReader reader, IValueTextWriter writer)
      throws IOException {
    ioHelperText2(io + " Integers", reader, writer,
        new DataGenerator(generator, INumber.class, intTestSet, new RandomIntegerGenerator(vf)));
    ioHelperText2(io + " Rationals", reader, writer,
        new DataGenerator(generator, INumber.class, ratTestSet, new RandomRationalGenerator(vf)));
    ioHelperText2(io + " Reals", reader, writer,
        new DataGenerator(generator, INumber.class, realTestSet, new RandomRealGenerator(vf)));
  }

  private void ioHelperBin(String io, IValueBinaryReader reader, IValueBinaryWriter writer)
      throws IOException {
    ioHelperBin2(io + " Integers", reader, writer,
        new DataGenerator(generator, INumber.class, intTestSet, new RandomIntegerGenerator(vf)));
    ioHelperBin2(io + " Rationals", reader, writer,
        new DataGenerator(generator, INumber.class, ratTestSet, new RandomRationalGenerator(vf)));
    ioHelperBin2(io + " Reals", reader, writer,
        new DataGenerator(generator, INumber.class, realTestSet, new RandomRealGenerator(vf)));
  }

  private void ioHelperText2(String typeName, IValueTextReader reader, IValueTextWriter writer,
      DataGenerator g) throws IOException {
    if (noisy)
  ...
  private void ioHelperBin2(String typeName, IValueBinaryReader reader, IValueBinaryWriter writer,
      DataGenerator g) throws IOException {
    if (noisy)
      System.out.printf("  %-16s ", typeName + ":");
    int count = 0;
  ...
  /**
   * Run all axioms in the current class, i.e. all *public* methos with names starting with "axiom".
   * 
   * Constructs argument lists using the data generator, and calls the axioms using reflection.
   * 
   * @throws Throwable
   */
  @Test
    ...
    for (Method m : methods) {
      if (m.getName().startsWith("axiom")) {
        ...
        if (hasINumber(params)) {
          ...
          callAxiom(m, params, new Object[params.length], 0, new DataGenerator(generator,
              INumber.class, intTestSet, new RandomIntegerGenerator(vf)));
          if (noisy)
            System.out.print(" " + count + " calls\n" + m.getName() + "\n  Rationals: ");
          callAxiom(m, params, new Object[params.length], 0, new DataGenerator(generator,
              INumber.class, ratTestSet, new RandomRationalGenerator(vf)));
          if (noisy)
            System.out.print(" " + count + " calls\n" + m.getName() + "\n  Reals:     ");
          callAxiom(m, params, new Object[params.length], 0, new DataGenerator(generator,
              INumber.class, realTestSet, new RandomRealGenerator(vf)));
          if (noisy)
            System.out.print(" " + count + " calls\n" + m.getName() + "\n  Mixed:     ");
          callAxiom(m, params, new Object[params.length], 0, new DataGenerator(generator,
              INumber.class, mixedTestSet, new RandomNumberGenerator(vf)));
        } else {
          if (noisy)
            System.out.print(m.getName() + "\n          :  ");
  ...
  /**
   * Restrict the number of random values for long argument lists, or we'll end up with billions of
   * calls.
   * 
   * @return the number of random values we should generate for an argument list of the given
   *         length.
   */
